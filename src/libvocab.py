...
def termlink(text):
    result = re.sub(r"<code>foaf:(\w+)<\/code>",
                    r"<code><a href='#term_\g<1>'>\g<1></a></code>", text)
    return result
...
class Term(object):
    ...
    def is_external(self, vocab):
        print("Comparing property URI ", self.uri,
              " with vocab uri: " + vocab.uri)
        return (False)
...
class Vocab(object):
    ...
    def _set_uri(self, value):
        # we don't want Namespace() objects and suchlike, but we can use them without whining.
        v = str(value)
        if ':' not in v:
...
class VocabReport(object):
    ...
    def generate(self, htmlgroups):
        ...
        tpl = tpl.replace("%concepttypes2%",
                          self.concepttypes2)
        tpl = tpl.replace("%concepttypes3%",
                          self.concepttypes3)
    ...
    def termlist(self):
        ...
        for term in self.vocab.classes:
            ...
            q = 'SELECT ?d ?l WHERE {?d rdfs:domain <%s> . ?d rdfs:label ?l } ' % (
                term.uri)
            ...
            if contentStr != "":
                domainsOfClass = "%s <td> %s </td></tr>" % (
                    startStr, contentStr)

            # class in range of -> only for classes included in this ontology specification
            q2 = 'SELECT ?d ?l WHERE {?d rdfs:range <%s> . ?d rdfs:label ?l } ' % (
                term.uri)
            ...
            q = 'SELECT ?sc ?l WHERE {<%s> rdfs:subClassOf ?sc . ?sc rdfs:label ?l } ' % (
                term.uri)
            ...
            for (row) in relations:
                ...
                if (subclass.find(str(self.vocab._get_uri())) < 0):
                    if (colon > 0):
                        ...
                        q2 = 'SELECT ?orsc ?or ?orv WHERE { <%s> rdfs:subClassOf ?orsc . ?orsc rdf:type <http://www.w3.org/2002/07/owl#Restriction> . ?orsc ?or ?orv }' % (
                            term.uri)
                            ...
                            if (str(orp) == "http://www.w3.org/2002/07/owl#onProperty"):
                                ...
                                if (orpcounter == 0):
                                    ...
                                    if (str(orp) == "http://www.w3.org/2002/07/owl#maxCardinality"):
                                        orscope2 = "at most"
                                    print(
                                        "write another cardinality of restriction")
                ...
                if contentStr != "":
                    subClassOf = "%s <td> %s </td></tr>" % (
                        startStr, contentStr)

                if contentStr2 != "":
                    restriction = "%s <td> %s </td></tr>" % (
                        startStr2, contentStr2)

            # class has sub class -> handles only "real" super classes
            hasSubClass = ''

            q = 'SELECT ?sc ?l WHERE {?sc rdfs:subClassOf <%s>. ?sc rdfs:label ?l } ' % (
                term.uri)
            relations = g.query(q)
            ...
            for (row) in relations:
                ...
                if contentStr != "":
                    hasSubClass = "%s <td> %s </td></tr>" % (
                        startStr, contentStr)

            # is defined by
            classIsDefinedBy = ''

            q = 'SELECT ?idb WHERE { <%s> rdfs:isDefinedBy ?idb  } ' % (
                term.uri)
            relations = g.query(q)
            startStr = '\n'
            ...
            if contentStr != "":
                classIsDefinedBy = "%s <tr><td> %s </td></tr>" % (
                    startStr, contentStr)

            # disjoint with
            isDisjointWith = ''

            q = 'SELECT ?dj ?l WHERE { <%s> <http://www.w3.org/2002/07/owl#disjointWith> ?dj . ?dj rdfs:label ?l } ' % (
                term.uri)
            relations = g.query(q)
            ...
            q = 'SELECT * WHERE { <%s> rdf:type <http://www.w3.org/2002/07/owl#Class> } ' % (
                term.uri)
            relations = g.query(q)
            ...
            q = 'SELECT * WHERE { <%s> rdf:type <http://www.w3.org/2000/01/rdf-schema#Class> } ' % (
                term.uri)
            relations = g.query(q)
            ...
            q = 'SELECT * WHERE { <%s> rdf:type <http://purl.org/dc/terms/AgentClass> } ' % (
                term.uri)
            relations = g.query(q)
            ...
            try:
                f = open(filename, "r")
                fileStr = f.read()
                fileStr = "<h4><a name=\"queries_" + term.id + "\"></a>" + term.id + \
                    " Validation Query</h4><pre>" + cgi.escape(ss) + "</pre>"
            except:
                fileStr = ''
            ...
            zz = eg % (term.id, term.uri, term.type, "Class", sn, term.label, term.comment, term.status, domainsOfClass, rangesOfClass +
                       subClassOf + restriction + hasSubClass + classIsDefinedBy + isDisjointWith + oc + rc + dctac, s, term.id, term.id)
        ...
        for term in self.vocab.properties:
            ...
            g = self.vocab.graph
            q = 'SELECT ?d ?l WHERE {<%s> rdfs:domain ?d . ?d rdfs:label ?l } ' % (
                term.uri)
            ...
            for (row) in relations:
                ...
                if (domain.find(str(self.vocab._get_uri())) < 0):
                    if colon > 0:
                        ...
                        q2 = 'SELECT ?d ?url ?urli ?urlipt WHERE {<%s> rdfs:domain ?d . ?d <http://www.w3.org/2002/07/owl#unionOf> ?url . ?url ?urlipt ?urli } ' % (
                            term.uri)
                        ...
                        for (domain, list, listitem, listitempropertytype) in relations2:
                            ...
                            if (str(listitempropertytype) == "http://www.w3.org/1999/02/22-rdf-syntax-ns#first"):
                                ...
                                urfirstlistitemnice = self.vocab.niceName(
                                    urfirstlistitem)
                        ...
                        if (urnextlistbnode != ""):
                            ...
                            while (str(urnextlistbnode) != "http://www.w3.org/1999/02/22-rdf-syntax-ns#nil"):
                                q3 = 'SELECT ?urnlbn ?urlipt ?urli WHERE {?lbn <http://www.w3.org/1999/02/22-rdf-syntax-ns#first> <%s> . ?lbn <http://www.w3.org/1999/02/22-rdf-syntax-ns#rest> ?urnlbn . ?urnlbn ?urlipt ?urli } ' % (
                                    urfirstlistitem)
                                ...
                                for (urnlbn, listitempropertytype, listitem) in relations3:
                                    print("what to do next with urnlbn ", urnlbn, " :: listitempropertytype ",
                                          listitempropertytype, " :: listitem ", listitem, " :: urnextlistbnode ", urnextlistbnode)
                                    ...
                                    if (str(urnlbn) == str(oldlistbnode)):
                                        ...
                                        if (str(listitempropertytype) == "http://www.w3.org/1999/02/22-rdf-syntax-ns#rest"):
                                            ...
                                            if (str(urnextlistbnode) != "http://www.w3.org/1999/02/22-rdf-syntax-ns#nil"):
                                                termStr3 = """<span about="[_:%s]" rel="rdf:rest" resource="[_:%s]"></span>""" % (
                                                    oldlistbnode, urnextlistbnode)
                                            else:
                                                termStr3 = """<span about="[_:%s]" rel="rdf:rest" href="%s"></span>""" % (
                                                    oldlistbnode, urnextlistbnode)
                                            print("new urnextlistbnode ",
                                                  urnextlistbnode)

                                contentStr2 = "%s or %s %s" % (
                                    contentStr2, termStr2, termStr3)
                        ...
                        contentStr3 = "%s %s %s" % (
                            contentStr3, termStr, contentStr2)

            # merge together the results of both queries
            if contentStr3 != "":
                contentStr = "%s %s" % (contentStr, contentStr3)
            if contentStr != "":
                domainsOfProperty = "%s <td> %s </td></tr>" % (
                    startStr, contentStr)

            # range of properties
            q2 = 'SELECT ?r ?l WHERE {<%s> rdfs:range ?r . ?r rdfs:label ?l } ' % (
                term.uri)
            relations2 = g.query(q2)
            ...
            for (row) in relations:
                ...
                if (range.find(str(self.vocab._get_uri())) < 0):
                    if colon > 0:
                        ...
                        q2 = 'SELECT ?r ?url ?urli ?urlipt WHERE {<%s> rdfs:range ?r . ?r <http://www.w3.org/2002/07/owl#unionOf> ?url . ?url ?urlipt ?urli } ' % (
                            term.uri)
            ...
            if contentStr3 != "":
                contentStr = "%s %s" % (contentStr, contentStr3)
            if contentStr != "":
                rangesOfProperty = "%s <td> %s </td></tr>" % (
                    startStr, contentStr)

            # property sub property of -> only for property included in this ontology specification
            subPropertyOf = ''

            q = 'SELECT ?sp ?l WHERE {<%s> rdfs:subPropertyOf ?sp . ?sp rdfs:label ?l } ' % (
                term.uri)
            ...
            if contentStr != "":
                subPropertyOf = "%s <td> %s </td></tr>" % (
                    startStr, contentStr)

            q1 = 'SELECT ?sp WHERE {<%s> rdfs:subPropertyOf ?sp } ' % (
                term.uri)
            ...
            q = 'SELECT ?sp ?l WHERE {?sp rdfs:subPropertyOf <%s>. ?sp rdfs:label ?l } ' % (
                term.uri)
            relations = g.query(q)
            ...
            if contentStr != "":
                hasSubProperty = "%s <td> %s </td></tr>" % (
                    startStr, contentStr)

            # property inverse property of -> only for property included in this ontology specification
            inverseOf = ''

            q = 'SELECT ?ip ?l WHERE {<%s> <http://www.w3.org/2002/07/owl#inverseOf> ?ip . ?ip rdfs:label ?l } ' % (
                term.uri)
            # print "term.uri ", term.uri
            relations = g.query(q)
            ...
            for (inverseproperty, label) in relations:
                ...
                if colon > 0:
                    ...
                    q2 = 'SELECT ?ipt WHERE {<%s> <http://www.w3.org/2002/07/owl#inverseOf> ?ip . ?ip rdfs:label ?l . ?ip rdf:type ?ipt } ' % (
                        term.uri)
                    relations2 = g.query(q2)
                    ...
                    for (row) in relations2:
                        ...
                        if (iptype != ""):
                            termStr3 = """<span about="[_:%s]" typeof="%s"><strong>%s</strong></span>""" % (
                                inverseproperty, iptypenice, iptype)
                            if (iptcounter > 0):
            ...
            q = 'SELECT ?ip ?l WHERE {?ip <http://www.w3.org/2002/07/owl#inverseOf> <%s>. ?ip rdfs:label ?l } ' % (
                term.uri)
            relations = g.query(q)
            ...
            if contentStr != "":
                hasInverseProperty = "%s <td> %s </td></tr>" % (
                    startStr, contentStr)
            ...
            if contentStr != "":
                propertyIsDefinedBy = "%s <tr><td> %s </td></tr>" % (
                    startStr, contentStr)

            # equivalent property
            equivalentProperty = ''

            q = 'SELECT ?ep WHERE { <%s> <http://www.w3.org/2002/07/owl#equivalentProperty> ?ep  } ' % (
                term.uri)
            relations = g.query(q)
            ...
            if contentStr != "":
                equivalentProperty = "%s <td> %s </td></tr>" % (
                    startStr, contentStr)
            ...
            q = 'SELECT * WHERE { <%s> rdf:type <http://www.w3.org/1999/02/22-rdf-syntax-ns#Property> } ' % (
                term.uri)
            relations = g.query(q)
            ...
            q = 'SELECT * WHERE { <%s> rdf:type <http://www.w3.org/2002/07/owl#ObjectProperty> } ' % (
                term.uri)
            relations = g.query(q)
            ...
            q = 'SELECT * WHERE { <%s> rdf:type <http://www.w3.org/2002/07/owl#DatatypeProperty> } ' % (
                term.uri)
            relations = g.query(q)
            ...
            q = 'SELECT * WHERE { <%s> rdf:type <http://www.w3.org/2002/07/owl#InverseFunctionalProperty> } ' % (
                term.uri)
            relations = g.query(q)
            ...
            q = 'SELECT * WHERE { <%s> rdf:type <http://www.w3.org/2002/07/owl#FunctionalProperty> } ' % (
                term.uri)
            relations = g.query(q)
            ...
            zz = eg % (term.id, term.uri, term.type, "Property", sn, term.label, term.comment, term.status, domainsOfProperty, rangesOfProperty + subPropertyOf +
                       hasSubProperty + inverseOf + hasInverseProperty + propertyIsDefinedBy + equivalentProperty + rp + op + dp + ifp + fp, s, term.id, term.id)
        ...
        if (len(self.vocab.individuals) > 0):
            ...
            for term in self.vocab.individuals:
                ...
                q = 'SELECT ?t ?l WHERE {<%s> rdf:type ?t. ?t rdfs:label ?l } ' % (
                    term.uri)
                relations = g.query(q)
                ...
                if contentStr != "":
                    individualIsDefinedBy = "%s <tr><td> %s </td></tr>" % (
                        startStr, contentStr)
                ...
                zz = ig % (term.id, term.uri, term.type, "Individual", sn, term.label,
                           term.comment, term.status, hasType + individualIsDefinedBy, s, term.id, term.id)
